...
public class MainPanel extends JPanel implements Runnable{
    ...
    static Piece newPiece = new Piece();
...
//        btnPause = new JButton("pause");
    ...
    public void update(){
        board.setGameOn(board.setFinish());
       // if(newPiece.getValue()!=8) board.setGameOn(BoardHelper.isGameFinished(board.getBoard()));
        if(board.isGameOn()&& board.isGameContinue()) {
            int col = getPoint(mouse.mx);
            int row = getPoint(mouse.my);
            if (row <= Board.ROWS && col <= Board.COLS) {
                if(board.isWhite&& board.AIplayer){
                    if(!board.isGameOn()) return;
                    //newPiece = GreedyPlayer.newMove(board,2);
                    newPiece= MinimaxAi.play(board);
                    System.out.println(""+ newPiece.getCol()+","+newPiece.getCol());
                    if (board.canPutPiece(newPiece.getRow(), newPiece.getCol(), 2)) {
                        board.getBoard()[newPiece.getRow()][newPiece.getCol()].setValue(newPiece.getValue());
                        board.checkBoard(2);
                        System.out.println("white");
                    }else System.out.println("white is fail");
                    board.isWhite= false;
                    board.isBlack= true;
                    board.WhiteContinue= false;
                }
                if (mouse.pressed) {
                    board.update(row, col);
                    newPiece= new Piece(row, col,1);
                    mouse.pressed = false;
                ...
                dudoan = BoardHelper.getPointMove(board.getBoard(), value);
                if(dudoan.isEmpty()) board.setGameOn(false);
            }
